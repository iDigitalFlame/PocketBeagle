...
/usr/bin/printf 'n -R root:root "/etc/tmpfiles.d"\n    chmod 0550 "${SYSCONFIG}/etc/tmpfiles.d"\n' >> "${SETUP_DIRECTORY}/bin/syslink"
/usr/bin/printf '    chmod 0440 "${SYSCONFIG}"/etc/tmpfiles.d/*\n    chmod 0550 "/etc/tmpfiles.d"' >> "${SETUP_DIRECTORY}/bin/syslink"
/usr/bin/printf '\n    chmod 0440 /etc/tmpfiles.d/*\nfi\nif [ -e "${SYSCONFIG}/etc/modprobe.d" ];' >> "${SETUP_DIRECTORY}/bin/syslink"
/usr/bin/printf ' then\n    chown -R root:root "/etc/modprobe.d"\n    chmod 0500 "${SYSCONFIG}/et' >> "${SETUP_DIRECTORY}/bin/syslink"
/usr/bin/printf 'c/modprobe.d"\n    chmod 0400 "${SYSCONFIG}"/etc/modprobe.d/*\n    chmod 0500 "/' >> "${SETUP_DIRECTORY}/bin/syslink"
/usr/bin/printf 'etc/modprobe.d"\nfi\nif [ -e "${SYSCONFIG}/etc/sysupdate.d" ]; then\n    chown -' >> "${SETUP_DIRECTORY}/bin/syslink"
/usr/bin/printf 'R root:root "/etc/sysupdate.d"\n    chmod 0550 "${SYSCONFIG}/etc/sysupdate.d"\n ' >> "${SETUP_DIRECTORY}/bin/syslink"
/usr/bin/printf '   chmod 0550 "${SYSCONFIG}"/etc/sysupdate.d/*\n    chmod 0550 "/etc/sysupdate.d' >> "${SETUP_DIRECTORY}/bin/syslink"
/usr/bin/printf '"\nfi\nif [ -e "${SYSCONFIG}/etc/nftables.conf" ]; then\n    chmod 0400 "${SYSCO' >> "${SETUP_DIRECTORY}/bin/syslink"
/usr/bin/printf 'NFIG}/etc/nftables.conf"\nfi\n\nif [ -d "/etc/syscheck.d" ]; then\n    for entry' >> "${SETUP_DIRECTORY}/bin/syslink"
/usr/bin/printf ' in /etc/syscheck.d/*.sh; do\n        if [ -f "$entry" ]; then\n            prin' >> "${SETUP_DIRECTORY}/bin/syslink"
/usr/bin/printf 'tf '\''Processing script "%%s"..\\n'\'' "$entry"\n            source "$entry"\n ' >> "${SETUP_DIRECTORY}/bin/syslink"
/usr/bin/printf '       fi\n    done\nfi\n\nsync\necho "Broken Links Found:"\nfind /etc -xtype l ' >> "${SETUP_DIRECTORY}/bin/syslink"
/usr/bin/printf '-xdev 2> /dev/null | grep -vE "/proc|/tmp|/run"\necho "Done!"\n' >> "${SETUP_DIRECTORY}/bin/syslink"
...
/usr/bin/printf '[Match]\nType                    = gadget\n\n[Link]\nActivationPolicy        = a' >> "${SETUP_DIRECTORY}/etc/systemd/network/usb0.network"
/usr/bin/printf 'lways-up\n\n[Network]\nDNS                     = 10.1.10.2\nAddress             ' >> "${SETUP_DIRECTORY}/etc/systemd/network/usb0.network"
/usr/bin/printf '    = 10.1.10.1/30\nIPv6AcceptRA            = no\nLinkLocalAddressing     = no\n' >> "${SETUP_DIRECTORY}/etc/systemd/network/usb0.network"
/usr/bin/printf '\n[Route]\nGateway                 = 10.1.10.1\n' >> "${SETUP_DIRECTORY}/etc/systemd/network/usb0.network"
